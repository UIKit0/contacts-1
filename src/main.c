...
e_util_unicode_get_utf8 (const char *text, gunichar *out)
{
        *out = g_utf8_get_char (text);
        ...
        return (G_UNLIKELY (*out == (gunichar) -1)) ? NULL : g_utf8_next_char (text);
}
...
static const char *
kozo_utf8_strstrcasestrip (const char     *haystack,
...
is_row_visible (GtkTreeModel *model, GtkTreeIter *iter, gpointer data)
{
	...
	const gchar *search_string = gtk_entry_get_text (GTK_ENTRY 
											(glade_xml_get_widget (xml, "search_entry")));
	...
	/* Search for any occurrence of the string in the search box in the contact
	 * file-as name; if none is found, row isn't visible. Ignores empty searches.
	 */
	if ((search_string) && (search_string[0] != '\0')) {
		gchar *name_string;		
		gtk_tree_model_get (model, iter, NAME, &name_string, -1);
		if (name_string) {
			gunichar *isearch = kozo_utf8_strcasestrip (search_string);
			if (!kozo_utf8_strstrcasestrip (name_string, isearch))
				result = FALSE;
			g_free (name_string);
			g_free (isearch);
		}
							...
							gpointer user_data)
{
	...
	returnval = strcasecmp ((const char *)string1, (const char *)string2);
	g_free (string1);
	g_free (string2);
...
update_treeview ()
{
	...
	model = GTK_TREE_MODEL_FILTER (gtk_tree_view_get_model (GTK_TREE_VIEW (
...
fill_treeview (GtkListStore *model)
{
	...
	if (status == FALSE) {
		...
		for (c = cards; c; c = c->next) {
			...
			gtk_list_store_set (model, &iter, NAME, file_as, ECONTACT, contact, -1);
			
			/* Check for groups and add them to group list */
			contact_groups = e_contact_get (contact, E_CONTACT_CATEGORY_LIST);
			if (contact_groups) {
				GList *group;
				for (group = contact_groups; group; group = group->next) {
					if (!g_list_find_custom (groups, group->data, 
													(GCompareFunc)strcmp)) {
						gtk_combo_box_append_text (groups_combobox, group->data);
						groups = g_list_prepend (groups, group->data);
					}
				}
 ...
 */
static void
set_label (const gchar *label_name, const gchar *name, const gchar *string)
{
							...
							gpointer user_data)
{
	...
	if (width > height)
		gdk_pixbuf_loader_set_size (loader, 64, height / ((gdouble)width/64.0));
	else
		gdk_pixbuf_loader_set_size (loader, width / ((gdouble)height/64.0), 64);
}
	...
	if (gtk_tree_selection_get_selected (selection, &model, &iter)) {
		...
		if (photo) {
			...
			g_signal_connect (G_OBJECT (loader), "size-prepared",
									G_CALLBACK (contact_photo_size), NULL);
...
main (int argc, char **argv)
{
   ...
   GtkTreeIter iter;
	...
	if (!xml)
		g_critical ("Could not find interface XML file '%s'", XML_FILE);
	
	/* Hook up signals defined in interface xml */
	glade_xml_signal_autoconnect(xml);
	
	/* Load the system addressbook */
	book_path = g_strdup_printf ("file://%s/.evolution/addressbook/local/system/", g_get_home_dir ());
	...
	contacts_vbox = GTK_VBOX (glade_xml_get_widget (xml, "contacts_vbox"));
	gtk_box_pack_start (GTK_BOX (contacts_vbox), GTK_WIDGET (groups_combobox),
								FALSE, TRUE, 6);
	...
	contacts_treeview = GTK_TREE_VIEW (
										glade_xml_get_widget (xml, "contacts_treeview"));
   ...
   filter = GTK_TREE_MODEL_FILTER (
   									...
   									NULL));
   gtk_tree_model_filter_set_visible_func (filter, is_row_visible, NULL, NULL);
   gtk_tree_view_set_model (GTK_TREE_VIEW (contacts_treeview),
                            GTK_TREE_MODEL (filter));
   /* Alphabetise the GtkTreeView */
   gtk_tree_sortable_set_default_sort_func (GTK_TREE_SORTABLE (model), sort_treeview_func, NULL, NULL);
